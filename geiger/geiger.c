...
// Global variables
EEMEM uint8_t beepDefault = 0xFF;   // power-on beep default stored in EEPROM
volatile uint8_t beep;              // flag used to mute beeper
...
void checkevent(void)
{
    if (eventflag) {        // a GM event has occurred, do something about it!
        ...
        if (beep) {                 // check if we're in beep mode
...
int main(void)
{
    ...
    beep = eeprom_read_byte(&beepDefault);      // read the current beep default from EEPROM
    if ((PIND & _BV(PD3)) == 0) {               // does the user want to change the beep default?
        beep = !beep;                           // yes, change it and write it back to EEPROM
        eeprom_write_byte(&beepDefault, beep);
    }
